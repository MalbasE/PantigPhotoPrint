...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['layout'], $_POST['paper-type'], $_POST['copies'], $_POST['printType'], $_POST['price'])) {
    ...
    if ($stmt->execute()) {
        header("Location: user_idpic.php?message=added");
        exit();
    } else {
...
<head>
    ...
    <title>KRISHIEL Printing and Imaging Services</title>
    <style>
   body {
...
<body>
    ...
    <!-- Flyers Section -->
    <form action="user_idpic.php" method="POST" enctype="multipart/form-data">
        <h2>ID Picture</h2>
    ...
    ?>
   <div class="col">
   <img src="../images/idpicture.png" alt="idpic" class="idpic">
        <div class="id_type">
            <div class="label">
            ...
            <div class="label">
                <label for="paper-type">ID Type:</label>
                <select class="option" name="paper-type" id="paper-type" required>
                <option selected value="Philippine Passport">Philippine Passport</option>
...
function calculatePrice() {
    ...
    const layoutPrices = {
        '1x1': 10,  // example price for 1x1 layout
        '2x2': 12,  // price for 2x2 layout
        '3x3': 15,  // price for 3x3 layout
        '4x4': 20   // price for 4x4 layout
    };

    // Price adjustments based on paper type
    const paperTypePrices = {
        'Philippine Passport': 2,
        'Driver\'s License': 2.5,
        'Unified Multi-Purpose ID (UMID)': 3,
        'Voter\'s ID': 1.5,
        'Postal ID': 1.8,
        'PhilHealth ID': 2,
        'Senior Citizen ID': 2.2,
        'Student ID': 1.7,
        'Barangay ID': 1.5
    };
