...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['layout'], $_POST['paper-type'], $_POST['copies'], $_POST['printType'], $_POST['price'])) {
    ...
    if ($stmt->execute()) {
        header("Location: user_sticker.php?message=added");
        exit();
    } else {
...
<body>
    <!-- Header Section -->
    <form action="user_sticker.php" method="POST" enctype="multipart/form-data">
    <div class="header">
        <div class="left-sec">
    ...
    ?>
   <div class="col">
   <img src="../images/sticker.png" alt="idpic" class="idpic">
        <div class="id_type">
            <div class="label">
            ...
            <div class="label">
                ...
                <option selected value="Vinyl">Vinyl Sticker Paper</option>
                <option value="Glossy">Glossy Sticker Paper</option>
                <option value="Matte">Matte Sticker Paper</option>
                <option value="Clear">Clear Sticker Paper</option>
                <option value="Paper">Paper Sticker Sheets</option>
                </select>
            </div>
            <div class="label">
            ...
            <?php if (!empty($cart_items)): ?>
                <?php foreach ($cart_items as $item): ?>
                    <li id="cart-item-<?php echo $item['order_id']; ?>" style="display: flex; align-items: center; margin-bottom: 10px;">
                        ...
                        <p><?php echo htmlspecialchars($item['service_type']); ?></p>
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    serviceButtons.forEach(button => {
        button.addEventListener('click', function() {
            // Capture the service name from the clicked link text and trim any extra spaces
            const serviceText = this.textContent.trim();

            console.log(serviceText); // Log to debug the text being captured

            // Set the hidden field value based on the service text dynamically
...
function calculatePrice() {
    ...
    const paperTypePrices = {
        'Vinyl': 10,   // Price in PHP
            'Glossy': 15,  // Price in PHP
            'Matte': 20,   // Price in PHP
            'Clear': 25,   // Price in PHP
            'Paper': 30     // Price in PHP
    };
