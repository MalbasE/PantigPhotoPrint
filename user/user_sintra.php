...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['layout'], $_POST['paper-type'], $_POST['copies'], $_POST['printType'], $_POST['price'])) {
    ...
    if ($stmt->execute()) {
        header("Location: user_sintra.php?message=added");
        exit();
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['checkout'])) {
    ...
    try {
        ...
        header("Location: user_sintra.php?message=checkout-success");
        exit();
    } catch (PDOException $e) {
...
<body>
    ...
    <!-- Flyers Section -->
    <form action="user_sintra.php" method="POST" enctype="multipart/form-data">
        <h2>Photo Sintra Board</h2>
    ...
    ?>
   <div class="col">
   <img src="../images/gradpic.jpg" alt="idpic" class="idpic">
        <div class="id_type">
            <div class="label">
                <label for="layout">Size:</label>
                <select class="option" name="layout" id="layout" required>
                    <<option selected value="4x6">4x6 inches</option>
            ...
            <div class="label">
                <label for="paper-type">Thickness:</label>
                <select class="option" name="paper-type" id="paper-type" required >
                    <option selected value="3mm">3mm (1/8 inch)</option>
                    <option value="5mm">5mm (3/16 inch)</option>
                    <option value="10mm">10mm (3/8 inch)</option>
...
function calculatePrice() {
    ...
    const layoutPrices = {
        '4x6': 180,  // ₱180.00 for Sintra photo 4x6 inches
                '5x7': 250,  // ₱250.00 for Sintra photo 5x7 inches
                '8x10': 350, // ₱350.00 for Sintra photo 8x10 inches
                '11x14': 450, // ₱450.00 for Sintra photo 11x14 inches
                '16x20': 700, // ₱700.00 for Sintra photo 16x20 inches
                '18x24': 900, // ₱900.00 for Sintra photo 18x24 inches
                '24x36': 1200 // ₱1200.00 for Sintra photo 24x36 inches
    };

    // Price adjustments based on paper type
    const paperTypePrices = {
        '3mm': 0,   // No additional cost
        '5mm': 50,  // ₱50 additional cost
        '10mm': 100 // ₱100 additional cost
    };
